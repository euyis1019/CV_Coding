...
def lr_poly(base_lr, iter, max_iter, power):#多项式衰减学习率
    """base_lr:基础学习率。
iter:当前迭代次数。
max_iter:最大迭代次数。
power:多项式的幂次。"""
...
def weightedMSE(D_out, D_label):#加权均方误差
    """
    D_out: Output, maybe a tensor (把具体维度写出来)
    D_label:(把具体维度写出来)
    """
...
def main():
    ...
    #cudnn的一些优化
    cudnn.enabled = True #关于一些操作运算
    cudnn.benchmark = True #输入数据的维度一定
    #，这个调优器会自动寻找最适合当前配置（如输入数据的大小和层的参数）的算法来提高效率
    ...
    #加载预训练模型的权重， torch.load的形式是什么样的
    saved_state_dict = torch.load(args.restore_from)

    #保存每一层的训练参数，eg: key--conv1 value:[32, 3, 3, 3] 一个张量，这里只是这个张量的size
    ...
    #调整不同域的光照分布，降低三个域因光线而导致的差异
    #即：减少后续语义分割网络对光照变化的敏感度。
    lightnet = LightNet()
    lightnet.train()
    lightnet.to(device)
    """
    ...
    """DataLoader是一个可迭代的数据装载器, ---batch
    组合了数据集和采样器，并在给定数据集上提供可迭代对象。
    可以完成对数据集中多个对象的集成。"""
    ...
    weights = torch.log(torch.FloatTensor([0.36869696, 0.06084986, 0.22824049, 0.00655399, 0.00877272, 0.01227341,
                                           ...
                                           0.00413907])).cuda()
    #标准化，并确保这个权重尽可能大于0 论文中 std=0.05 avg = 1.0 (经验)
    ...
    loss_TV = L_TV() #total variation 降噪--make images smoother
    loss_SSIM = SSIM()# 
    ...
    for i_iter in range(args.num_steps):
        ...
        for sub_i in range(args.iter_size):
            ...
            if args.model == 'RefineNet':
                pred_target_d = model(enhanced_images_d)#白天的目标域预测值
            else:
            ...
            # 基于：Learning to adapt structured output space for semantic segmentation
