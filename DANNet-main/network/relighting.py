...
class ResnetGenerator(nn.Module):
    # 这是一个基于ResNet块的生成器，它在一些下采样/上采样操作之间包含多个ResNet块。
    # 这段代码和思路是基于Justin Johnson的快速神经风格迁移项目的Torch代码适配而来。
    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        #   ngf (int) - 最后一个卷积层的过滤器数目
        #   norm_layer - 使用的归一化层
        #   use_dropout (bool) - 是否使用dropout层
        #   n_blocks (int) - ResNet块的数量
        #   padding_type (str) - 卷积层中填充层的类型：reflect | replicate | zero
        assert(n_blocks >= 0)
        super(ResnetGenerator, self).__init__()
        use_bias = (norm_layer == nn.InstanceNorm2d)

        # 定义模型的初始层，使用反射填充和7x7的卷积
        model = [nn.ReflectionPad2d(3),#填充边界
                 nn.Conv2d(input_nc, ngf, kernel_size=7, padding=0, bias=use_bias),#生成ngf个初始卷积
                 norm_layer(ngf),
                 nn.ReLU(True)]
